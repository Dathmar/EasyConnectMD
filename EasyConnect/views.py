...
from EasyConnect.forms import PatientForm, SymptomsForm, ProviderForm, PaymentForm, ICD10CodeLoad, \
    AffiliateForm, CouponForm, VitalsForm, PrescriptionProvidedForm
from EasyConnect.models import Patient, Symptoms, ProviderNotes, Appointments, Payment, Icd10, \
    Affiliate, Coupon, Patient_Cost, Vitals, UserPhone
...
def connect_2_affiliate(request, patient_id, affiliate_url):
    ...
    if request.method == 'POST':
        ...
        if symptom_form.is_valid() and vitals_form.is_valid() and (payment_form.is_valid() or patient_cost <= 0):
            ...
            if vitals:
                vitals.height_feet = height_feet
                vitals.height_inches = height_inches
            ...
            symptoms = Symptoms.objects.filter(patient_id=patient_id)

            if symptoms:
                symptoms.pancreatitis_thyroid_cancer = pancreatitis_thyroid_cancer,
                symptoms.allergies = allergies,
                symptoms.medications = medications,
                symptoms.previous_diagnosis = previous_diagnosis,
...
def provider_view(request, patient_id):
    ...
    vitals = Vitals.objects.filter(patient_id=patient_id).first()

    bmi = (vitals.weight * 703) / (vitals.height_feet * 12 + vitals.height_inches)**2
    ...
    if request.method == 'POST':
        ...
        prescription_form = PrescriptionProvidedForm(request.POST)

        if provider_form.is_valid() and prescription_form.is_valid():
            ...
            logger.info(f'Saving assessment {assessments}')

            existing_notes = ProviderNotes.objects.filter(patient=patient)

            if not existing_notes and prescription:
                logger.info(f'{patient_id} sending prescription notification.')
                send_appointment_ending(patient_id)

            provider_notes = ProviderNotes(patient=patient,
                                           hpi=hpi,
                                           treatment=treatment,
                                           followup=followup,
                                           assessments=Icd10.objects.get(id=assessments),
    ...
    # If this is a GET (or any other method) create the default form.
        ...
        provider_notes = ProviderNotes.objects.filter(patient_id=patient_id).order_by('-create_datetime').first()
        prescription_form = PrescriptionProvidedForm()
        if provider_notes:
            if provider_notes.assessments:
                logger.info('has assessment')
                provider_form = ProviderForm(initial={'hpi': provider_notes.hpi,
                                                      ...
                                                      'assessments': Icd10.objects.get(id=provider_notes.assessments.id).id,
    ...
    same_patient = Patient.objects.filter(
        first_name=patient.first_name, last_name=patient.last_name, dob=patient.dob
    ).exclude(id=patient_id)

    patient_records = []
    if same_patient:
        logger.info(f'{patient_id} found matching previous appointments.')
        for record in same_patient:
            previous_symptoms = Symptoms.objects.filter(patient_id=record.id)
            previous_notes = ProviderNotes.objects.filter(patient_id=record.id)
            previous_vitals = Vitals.objects.filter(patient_id=record.id).first()
...
def get_patient_records(patient_id):
    sql = f'''
            ...
            SELECT
                ...
                --, vitals.height_feet height_feet, vitals.height_inches height_inches, vitals.weight weight
            FROM public."EasyConnect_providernotes" AS PN
                ...
                --INNER JOIN public."EasyConnect_vitals" vitals ON PN.patient_id=vitals.patient_id
...
def send_provider_notification(patient_id, reminder=False):
    ...
    provider_list = User.objects.filter(groups__name='Appointment Notifications').values('email')
    provider_texts = User.objects.filter(groups__name='Text Appointment Notifications')
    provider_numbers = UserPhone.objects.filter(user__in=provider_texts).values('phone_number')

    send_list = []
    for provider in provider_list:
        send_list.append(provider['email'])

    video_url = f'{settings.EMAIL_BASE_URL}/provider/{patient_id}'

    message = Mail(to_emails=send_list, from_email='patient@kellumconnectmd.com')

    if reminder:
        message.template_id = 'd-ef5e4b7b5be5420389efe7c432459ddb'
        text_body = f'{patient} is still waiting {video_url}'
    else:
        message.template_id = 'd-bab6b8b911fe49f8a1af01ca78f7d220'
        text_body = f'New video chat for {patient} {video_url}'
    ...
    if provider_numbers:
        ...
        for number in provider_numbers:
            logger.info(f'sending text to {number["phone_number"]}')
    ...
    logger.info(f'{patient_id} sending provider e-mail')
    send_sendgrid_message(message, patient_id)


def send_appointment_ending(patient_id):
    patient = Patient.objects.filter(pk=patient_id).first()

    message = Mail(to_emails=patient.email, from_email='patient@kellumconnectmd.com')
    message.template_id = 'd-b1a4bbaca1264368b6a553c245103664'
    message.dynamic_template_data = {
        'first_name': patient.first_name,
    }
    send_sendgrid_message(message, patient_id)
...
def send_patient_notification(patient_id):
    ...
    if int(patient_cost.cost) > 0:
        ...
        payment_date = str(payment_info['payment']['created_at'])
        ...
        payment_date = str(datetime.today())
    ...
    message = Mail(to_emails=patient.email, from_email='patient@kellumconnectmd.com')
    message.template_id = 'd-77619b7e255e45b08b5089b3b0241332'
    message.dynamic_template_data = {
        'first_name': patient.first_name,
        'video_url': video_url,
...
def send_sendgrid_message(message, patient_id):
    sg = SendGridAPIClient(settings.SEND_GRID_API_KEY)
    message.add_bcc('z.dickson@kellummd.com')
    try:
        response = sg.send(message)
        logger.info(f'{patient_id} e-mail sent')
        logger.info(f'{response}')
    except HTTPError as e:
        logger.info(f'{patient_id} e-mail HTTPError')
        logger.info(f'{e.to_dict}')
    except Exception as e:
        logger.info(f'{patient_id} e-mail other exception {e}')
